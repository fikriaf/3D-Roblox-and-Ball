...
# Small "2" on FRONT - MIRRORED for correct projection view
front_z = body_front
small_num_height = round(7 * scale)
small_num_width = round(5 * scale)
small_num_y_start = body_top + round(body_height * 0.15)
small_num_x_start = body_left + round(8 * scale)  # Mirrored position (left side)
stroke = max(1, round(1 * scale))
small_mid = round(3 * scale)

# Mirrored "2": left stroke at top, right stroke at bottom
...
voxel[small_num_y_start:small_num_y_start+small_mid, small_num_x_start:small_num_x_start+stroke, front_z:front_z+text_depth, :] = warna_hitam[:]  # Left top (mirrored)
...
voxel[small_num_y_start+small_mid:small_num_y_start+small_num_height, small_num_x_start+small_num_width-stroke:small_num_x_start+small_num_width, front_z:front_z+text_depth, :] = warna_hitam[:]  # Right bottom (mirrored)
...
# Flip horizontal to match projection view
slice[:, :, :] = voxel[:, :, body_front + 1, :]
slice_flipped = np.flip(slice, axis=1)  # Flip horizontal
plt.figure(6); plt.title('Front Body View - Small 2 (Projection Preview)')
plt.imshow(slice_flipped)
